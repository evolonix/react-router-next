...
- **`@evolonix/react-router-next/vite`** — the `reactRouterNext` Vite plugin, exported as the default (the named `reactRouterNext` and older `routeTypegen` exports are retained as deprecated aliases), and a programmatic `generateRouteTypes` API.
